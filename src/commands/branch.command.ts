 ...
 * slug is generated by the LLM from the issue content (mechanical title
 * slug as fallback). The issue-<n>- segment is the shape rig pr parses,
 * so the PR auto-links later.
 *
 * Because the LLM slug is not deterministic, any existing branch for the
 * issue is reused (switched to) instead of creating a second branch.
 */
export class BranchCommand extends BaseCommand {
  ...
  async execute(issueArg: string): Promise<void> {
    ...
    try {
      ...
      const slug = await this.resolveSlug(issue);
  ...
  /**
   * Resolves the branch slug: LLM-generated when the provider is
   * available, mechanical title slug otherwise or on LLM failure.
   *
   * @param issue - The issue to name the branch for
   * @returns A kebab-case slug
   */
  private async resolveSlug(issue: Issue): Promise<string> {
    if (await this.llm.isAvailable()) {
      try {
        return await this.logger.spinner(
          this.llm.suggestBranchSlug(issue.title, issue.body),
          `Naming branch with ${this.llm.providerName}...`
        );
      } catch (error) {
        this.logger.warn(
          `${this.llm.providerName} naming failed (${(error as Error).message}); using title slug.`
        );
      }
    }
    return this.slugFromTitle(issue.title);
  }
