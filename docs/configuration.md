...
**Type:** `array<ValidLabel>`
**Default:** `[]`

Default labels to automatically apply when creating issues via `rig create-issue`.

**Valid labels are defined in `src/types/labels.types.ts` as the single source of truth.**
...
- `devnet` - Development network component
- `node` - Node.js component
- `infra` - Infrastructure component
- `serverless` - Serverless component
...
**Phase labels:**
- `Phase 1: MVP` - MVP phase
- `Phase 2: Enhancement` - Enhancement phase
...
**Special labels:**
- `rig-generated` - Generated by rig-cli
- `rig-created` - Created via rig create-issue

**Notes:**
- Labels are validated against `src/types/labels.types.ts` before issue creation
- Invalid label names will be rejected with a helpful error message
- Labels must already exist in your GitHub repository (rig validates against the defined set, but GitHub must have them created)
- Empty array or omitted field means no labels are applied
- To add new valid labels, update `src/types/labels.types.ts`
...
# Default labels for created issues (must be valid labels from src/types/labels.types.ts)
defaultLabels:
  - rig-generated
  - needs-review
